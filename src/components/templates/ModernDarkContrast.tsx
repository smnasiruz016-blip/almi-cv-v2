...
export function ModernDarkContrast({
  data,
  paginated,
}: {
  data: CVData;
  ...
  return (
    ...
    >
      {/* Left Full-Bleed Dark Sidebar (4-Col) */}
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col items-center text-center">
            {basics.photoUrl ? (
              ...
              <div className="h-28 w-28 rounded-2xl border-2 border-dashed border-emerald-400/60 bg-slate-800 flex flex-col items-center justify-center p-2 shadow-inner">
                ...
                <span className="text-[9px] uppercase tracking-wider text-slate-400 mt-1">+ Photo</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-700/70">
            ...
            <div className="flex flex-wrap gap-1.5">
              {(skills.length > 0 ? skills : ["Cloud Architecture", "Distributed Systems", "Team Leadership", "Strategic Planning", "DevSecOps"]).map(
            ...
            {experience.map((job: any, i: number) => (
              <div key={job.id || i} className="border-l-2 border-slate-200 pl-3.5 space-y-1">
                <div className="flex justify-between items-baseline">
                  <h3 className="text-sm font-bold text-slate-900">{job.role || job.title || "Executive Role"}</h3>
