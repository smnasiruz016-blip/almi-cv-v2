...
export function SoftPastelFloatingCards({ data, paginated }: TemplateProps) {
  ...
  return (
    ...
    >
        ...
        <div className="shrink-0">
          {basics.photoUrl ? (
            ...
            <div className="h-22 w-22 rounded-2xl border-2 border-dashed border-violet-300 bg-violet-50 flex flex-col items-center justify-center text-center p-2">
              ...
              <span className="text-[8px] uppercase tracking-tight text-violet-400 mt-0.5">
                + Photo
              </span>
            </div>
          )}
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div
          className={`${
            basics.summary ? "col-span-5" : "col-span-12"
          } bg-white rounded-2xl p-5 shadow-sm border border-slate-200/80`}
        >
          <h2 className="text-xs font-bold uppercase tracking-wider text-violet-600 mb-2 flex items-center gap-1">
            ✦ Core Competencies
          </h2>
          <div className="flex flex-wrap gap-1.5">
            {(skills.length > 0
              ? skills
              : ["Talent Strategy", "Employee Relations", "Culture", "Retention", "Onboarding", "Conflict Resolution"]
            ...
            <div key={job.id || i} className="border-l-2 border-violet-200 pl-3.5 space-y-1">
              <div className="flex justify-between items-baseline">
                <h3 className="text-sm font-bold text-slate-900">
                  {job.role || job.title || "Senior Partner"}
                </h3>
