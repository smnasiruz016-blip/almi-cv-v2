...
export function WarmCreativeMagazine({
  data,
  paginated,
}: {
  data: CVData;
  ...
  return (
    ...
    >
      {/* Left Column (8-Col): Main Editorial Body */}
      <div className="col-span-8 p-8 space-y-6">
        ...
        {basics.summary && (
          <section>
            ...
            <p className="text-xs leading-relaxed text-[#44403c] text-justify font-sans">{basics.summary}</p>
          </section>
        )}
            ...
            {experience.map((job: any, i: number) => (
              <div key={job.id || i} className="space-y-1">
                <div className="flex justify-between items-baseline">
                  <h3 className="text-sm font-bold font-serif text-[#431407]">{job.role || job.title || "Senior Lead"}</h3>
            ...
            <div className="space-y-2 text-xs font-sans">
              {education.map((edu: any, index: number) => (
                <div key={edu.id || index}>
                  <div className="font-bold text-[#431407] font-serif">{edu.degree}</div>
                  <div className="text-[#78716c]">{edu.institution}</div>
                  <div className="text-[10px] text-[#a8a29e]">{edu.year || edu.gradYear}</div>
                </div>
              ))}
            </div>
          </section>
      ...
      {/* Right Column: Terracotta Warm Block (4-Col) */}
      <div className="col-span-4 bg-[#7c2d12] text-[#ffedd5] p-6 space-y-6 font-sans flex flex-col justify-between">
        <div className="space-y-6">
          ...
          <div className="flex flex-col items-center">
            {basics.photoUrl ? (
              ...
              <div className="h-28 w-28 rounded-full border-2 border-dashed border-[#fdba74]/60 bg-[#9a3412] flex flex-col items-center justify-center p-2">
                <span className="text-2xl font-black text-[#fdba74] font-serif">{initials}</span>
                <span className="text-[9px] uppercase tracking-wider text-[#fed7aa] mt-1">+ Photo</span>
              </div>
            )}
          </div>
          ...
          {/* Rating Dots Style Skills */}
          <div className="space-y-2 pt-2 border-t border-[#9a3412]">
            <div className="text-[10px] font-bold uppercase tracking-widest text-[#fdba74]">
              Skills & Tools
            </div>
            <div className="space-y-1.5 text-xs">
              {(skills.length > 0 ? skills : ["Art Direction", "Typography", "Editorial Layout", "Color Theory", "Photography"]).map(
          ...
          <div className="pt-2 border-t border-[#9a3412] text-xs">
            ...
            {languages.map((l: any, i: number) => (
              <div key={i} className="flex justify-between text-[11px]">
                ...
                <span className="text-[#fed7aa]">{l.fluency || l.level || "Fluent"}</span>
              </div>
            ))}
          </div>
        )}
