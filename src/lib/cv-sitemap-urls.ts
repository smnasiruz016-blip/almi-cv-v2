...
export function buildAllCvUrls(): MetadataRoute.Sitemap {
  ...
  // /ai-cv-builder/[country] AI-angle landing pages — same data + gate,
  // distinct ("ai cv builder in [country]") query angle.
  for (const c of COUNTRIES_SERVED) if (hasFreeCvContent(c.slug)) out.push({ url: `${SITE_ORIGIN}/ai-cv-builder/${c.slug}`, lastModified: now, changeFrequency: "weekly", priority: 0.8 });
